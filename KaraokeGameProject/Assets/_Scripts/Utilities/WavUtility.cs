...
public class WavUtility
{
    ...
    const string RECORDINGS_DIRECTORY_NAME = "Recordings";
    ...
    public static byte[] FromAudioClip(AudioClip audioClip, out string filePath, bool saveAsFile = true, string dirName = "recordings")
    {
        ...
        // Save file to persistent storage location
        if (saveAsFile)
        {
    ...
    /// <summary>
    /// Save data of an audio clip to wav file.
    /// </summary>
    /// <param name="audioClip">Audio clip contains the to be saved data.</param>
    /// <param name="filePath">Path of the saved file when the save operation is done.</param>
    /// <param name="storageLocation">Save location of the file. If empty, default location will be "Application.persistentDataPath" and stored in the "Recordings" directory.</param>
    /// <param name="fileName">Name of the saved file. If empty, default name will be the time when the record is saved (Ex: "240426-195551-710").</param>
    /// <param name="saveAsFile">Whether the data in the audio clip should be saved in persistent location. If false, the data will only be converted to byte array.</param>
    /// <returns></returns>
    public static byte[] FromAudioClip(AudioClip audioClip, out string filePath, string storageLocation = "", string fileName = "", bool saveAsFile = true)
    {
        ...
        // total file size = 44 bytes for header format and audioClip.samples * factor due to float to Int16 / sbyte conversion
        int fileSize = audioClip.samples * BlockSize_16Bit + WAV_HEADER_SIZE; // BlockSize (bitDepth)
        ...
        if (saveAsFile)
        {
            // Use default storage location if storage location is not specified.
            if (storageLocation == string.Empty || storageLocation is null)
            {
                storageLocation = Path.Combine(Application.persistentDataPath, RECORDINGS_DIRECTORY_NAME);
            }
            // Set file name to default name if file name is not specified.
            if (fileName == string.Empty || fileName is null)
            {
                fileName = DateTime.UtcNow.ToString("yyMMdd-HHmmss-fff");
            }

            filePath = Path.Combine(storageLocation, fileName + WAV_FILE_EXTENSION);
            WriteAndSaveWAVFile(filePath, bytes);
        }
        else
        {
            filePath = null;
    ...
    private static void WriteAndSaveWAVFile(string filePath, byte[] data)
    {
        try
        {
            // If file already exists in filePath, delete it
            if (File.Exists(filePath))
            {
                Debug.Log("Delete existing file at path: " + filePath);
        ...
        {
            Debug.LogError("Cannot find the directory in the file path: " + filePath);
        }
        catch (Exception e)
        {
